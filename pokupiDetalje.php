<h4><i class="fi-info"></i> Detalji narudžbe</h4>
	...
	$izraz = $veza -> prepare("select c.ime,c.prezime from narudzba a inner join zaposlenik b on a.zaposlenik=b.sifra inner join osoba c on b.osoba=c.sifra where a.sifra=:sifra ");
	$izraz->bindParam("sifra",$_GET["sifra"]);
	$izraz -> execute();
	...
	<td>
		<?php
		 if(empty($zaposlenik)){
		 	...
		 	echo $zaposlenik->ime." ".$zaposlenik->prezime; 
		 }

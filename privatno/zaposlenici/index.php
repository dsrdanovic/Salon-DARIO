...
$izraz = $veza -> prepare("select a.*,b.oib,b.ime,b.prezime,b.mobitel,b.email,c.aktivan from zaposlenik a inner join osoba b on a.osoba=b.sifra inner join operater c on a.operater=c.sifra where " . $preduvjet . " like :uvjet");
	...
	<body>
		...
		<div class="row">
			<div class="large-12 columns">
				...
				<h4><i class="fi-torsos-all"> </i>Zaposlenici</h4>
				<hr />
...
<?php foreach ($zaposlenik as $z): ?>
					

<tr>
	<td><?php echo $z -> sifra; ?></td>
	<td><?php echo $z -> oib; ?></td>
	<td><?php echo $z -> ime; ?></td>
	<td><?php echo $z -> prezime; ?></td>		
	<td><?php echo $z -> mobitel; ?></td>
	<td><?php echo $z -> email; ?></td>
	<td><?php echo $z -> radnomjesto; ?></td>
	<td><input id="checkbox1" type="checkbox" <?php
		if ($z -> aktivan == 1) { echo "checked";}?>></td>
	<td>
	...
	<li><a href="#" title="Pogledaj detalje"><i class="fi-zoom-in"></i></a></li>
  	<li><a href="promjeni.php?sifra=<?php echo $z->sifra; ?>" title="Promjeni"><i class="fi-loop"></i></a></li>
  	
<?php  
  	$izraz = $veza -> prepare('select count(sifra) as ukupno from narudzba where zaposlenik=:sifra');
$s = $z->sifra;
...
<?php endforeach; ?>
    ...
    $(link).attr('href', this.href);
 	...
 	var tekst = $('.tekst');
 	 $(tekst).text("Želite obrisati zaposlenika ("+this.id+") ?");
});
		</script>
